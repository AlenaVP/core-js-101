...
function getRectangleString(width, height) {
  const h = String.fromCharCode(9472).repeat(width - 2);
  const v = String.fromCharCode(9474);
  let rect = `${String.fromCharCode(9484)}${h}${String.fromCharCode(9488)}\n`;
  for (let i = 0; i < height - 2; i += 1) {
    rect += `${v}${' '.repeat(width - 2)}${v}\n`;
  }
  rect += `${String.fromCharCode(9492)}${h}${String.fromCharCode(9496)}\n`;
  return rect;
}
